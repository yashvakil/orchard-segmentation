...
def plot_log(file_dir, file_name, save=False, verbose=False):
    ...
    if save:
        fig.savefig(join(file_dir, str(file_name.split('.')[0]) + '.jpg'))
        if verbose:
            print("="*51)
            print("Logs Graph saved at " + str(join(file_dir, str(file_name.split('.')[0]) + '.jpg')))
            print("="*51)
...
if __name__ == "__main__":
    plot_log(args.logs_dir, "logs_19_01_25-12_05_15.csv", save=True, verbose=args.debug)