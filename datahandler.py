...
def load_satellite_images(path:str, satellite_image:str, label_image:str, save: bool = False, verbose: bool = False) -> Union[np.ndarray, np.ndarray]:
    """ Load the images form tif file

    :param satellite_image:
    :param label_image:
    :param save:
    :param verbose:
    ...
    satellite_image_path = join(path, satellite_image)
    label_image_path = join(path, label_image)

    satellite_image_whole = tif.imread(satellite_image_path)
    label_image_whole = tif.imread(label_image_path)

    if satellite_image_whole.ndim <= 2:
        satellite_image_whole = np.expand_dims(satellite_image_whole, axis=-1)
    else:
        satellite_image_whole = satellite_image_whole[:,:,0:3]

    if label_image_whole.ndim <= 2:
        label_image_whole = np.expand_dims(label_image_whole, axis=-1)
    else:
        label_image_whole = label_image_whole[:,:,0:3]

    if verbose:
        print("=" * 19 + "IMAGES LOADED" + "=" * 19)
    # assert satellite_image_whole.shape[:-2] == label_image_whole[:-2], "Images are not of the same shape"

    if verbose:
        print("=" * 20 + "IMAGES SHAPE" + "=" * 20)
        print("Satellite ".ljust(20) + "|" + str(satellite_image_whole.shape))
        print("Label".ljust(20) + "|" + str(label_image_whole.shape))
        print("=" * 52)

    satellite_image_whole[satellite_image_whole < 0] = 0.0
    label_image_whole[label_image_whole != 1] = 0.0
    ...
    if save:
        ...
        np.save(join(path, "label_image_whole.npy"), label_image_whole)
        if verbose:
            print("=" * 50)
            print("Images array saved at " + str(path))
            print("=" * 50)

    return satellite_image_whole, label_image_whole
...
def gen_patches(image: np.ndarray, label: np.ndarray, patch_size: int = 512, stride: int = 512,
    ...
    if save:
        ...
        makedirs(join(save_at, "Label"), exist_ok=True)
        ...
        for i in range(0, h_scaled - patch_size + 1, stride):
            for j in range(0, w_scaled - patch_size + 1, stride):
                ...
                if True:
                    ...
                    if save:
                        tif.imsave(join(save_at, "Satellite", "satellite_" + str(count) + ".tif"), x)
                        tif.imsave(join(save_at, "Label", "label_" + str(count) + ".tif"), y)
                    count = count + 1
                    ...
                    for k in range(0, aug_times):
                        ...
                        if save:
                            tif.imsave(join(save_at, "Satellite", "satellite_" + str(count) + ".tif"), augx)
                            tif.imsave(join(save_at, "Label", "label_" + str(count) + ".tif"), augy)
                        count = count + 1
        ...
        if save:
            ...
            np.save(join(save_at, "label_images.npy"), label_patches)
            if verbose:
                print("=" * 51)
        ...
        if verbose:
            ...
            print("Label".ljust(20) + "|" + str(label_patches.shape))
            print("=" * 52)
