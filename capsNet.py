...
def get_splits(verbose: bool =False):
    if not (exists(join(args.patch_dataset_dir, "x_train.npy"))
        ...
        if not (exists(join(args.patch_dataset_dir, "satellite_images.npy"))
            ...
            if not (exists(join(args.dataset, "satellite_image_whole.npy"))
                ...
                satellite_image_whole, label_image_whole = load_satellite_images(args.dataset, "satellite.tif", "orchard.tif", save=True,
                                                                                verbose=args.debug)
            else:
        ...
        satellite_images, label_images = shuffle_dataset((satellite_images, label_images), verbose=args.debug)

        test_size = int(satellite_images.shape[0] * 0.2)
        x_train = satellite_images[:-1 * test_size]
        x_val = satellite_images[-1 * test_size:]
        y_train = label_images[:-1 * test_size]
        y_val = label_images[-1 * test_size:]
    ...
    if verbose:
        ...
        print("Label (Training):".ljust(30) + " |" + str(y_train.shape))
        print("Satellite (Validation):".ljust(30) + " |" + str(x_val.shape))
        print("Label (Validation):".ljust(30) + " |" + str(y_val.shape))
        print("=" * 59)
...
def build_arch3(input_shape, n_class=2):
    ...
    # Layer 3: Convolutional Capsule
    conv_cap_3_2 = ConvCapsuleLayer(kernel_size=5, num_capsule=8, num_atoms=64, strides=4, padding='same',
                                    routings=3, name='conv_cap_3_2')(conv_cap_3_1)
    ...
    # Layer 1 Up: Deconvolutional Capsule
    deconv_cap_1_1 = DeconvCapsuleLayer(kernel_size=4, num_capsule=8, num_atoms=32, upsamp_type='deconv',
                                        scaling=4, padding='same', routings=3,
                                        name='deconv_cap_1_1')(conv_cap_4_1)
...
def build_arch4(input_shape, n_class=2):
    x = layers.Input(shape=input_shape)
    ...
    # Layer 2 Up: Deconvolutional Capsule
    deconv_cap_2_2 = ConvCapsuleLayer(kernel_size=5, num_capsule=4, num_atoms=64, strides=1,
                                      padding='same', routings=3, name='deconv_cap_2_2')(up_2)
    ...
    # Layer 4: Convolutional Capsule: 1x1
    seg_caps = ConvCapsuleLayer(kernel_size=1, num_capsule=1, num_atoms=64, strides=1, padding='same',
                                routings=3, name='seg_caps')(up_3)
    ...
    def shared_decoder(mask_layer):
        ...
        recon_2 = layers.Conv2D(filters=64, kernel_size=1, padding='same', kernel_initializer='he_normal',
                                activation='relu', name='recon_2')(recon_1)
